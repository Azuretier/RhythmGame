...
export default function BoardRenderer3D({
  ...
  const fireKeyMove = useCallback((rawInput: 'forward' | 'backward' | 'left' | 'right') => {
    if (cameraMode === 'fps') {
      ...
      if (rawInput !== 'backward') facingRef.current = worldDir;
      onMove(worldDir);
    } else {
  ...
  useEffect(() => {
    const keyToRaw = (key: string): 'forward' | 'backward' | 'left' | 'right' | null => {
      switch (key) {
    ...
    const handleBlur = () => { pressedInputs.current.clear(); };
    ...
    const pollInterval = setInterval(() => {
      ...
      for (const d of pressedInputs.current) rawInput = d;
      if (rawInput) fireKeyMove(rawInput);
    }, 16);
  ...
  return (
    <div className={styles.boardWrapper3d}>
      ...
      <div className={styles.dpad3d}>
        <button className={`${styles.dpadBtn3d} ${styles.dpadUp3d}`} onPointerDown={() => startDpadHold('up')} onPointerUp={clearDpadInterval} onPointerLeave={clearDpadInterval}>W</button>
        <button className={`${styles.dpadBtn3d} ${styles.dpadLeft3d}`} onPointerDown={() => startDpadHold('left')} onPointerUp={clearDpadInterval} onPointerLeave={clearDpadInterval}>A</button>
        <button className={`${styles.dpadBtn3d} ${styles.dpadDown3d}`} onPointerDown={() => startDpadHold('down')} onPointerUp={clearDpadInterval} onPointerLeave={clearDpadInterval}>S</button>
        <button className={`${styles.dpadBtn3d} ${styles.dpadRight3d}`} onPointerDown={() => startDpadHold('right')} onPointerUp={clearDpadInterval} onPointerLeave={clearDpadInterval}>D</button>
      </div>
    </div>
  );
}
