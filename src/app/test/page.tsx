...
const Main = () => {
  ...
  useEffect(() => {
    // Animate fadeUp div opacity on mount
    const fadeUp = document.getElementById("fadeUp")
    if (fadeUp != null) {
      animate(fadeUp, { opacity: [0.1, 1] })
    }

    // Set up interval to rotate text every 2 seconds
    const interval = setInterval(() => {
      setRotatingTextIndex((prev) => (prev + 1) % texts.length)
    }, 2000)
    ...
    <main className="grid grid-cols-12 grid-rows-6 grid-flow-row items-center justify-center h-screen bg-black">
      ...
      >
        ...
        <div className="grid gap-1 text-subtext row-start-3">
          ...
          <div className="flex gap-2 items-center">
            <FaPaperPlane />
            <span id="rotating-text">{texts[rotatingTextIndex]}</span>
          </div>
        </div>
      </motion.div>
    </main>
