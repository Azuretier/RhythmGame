...
type SyncPlayerState = Extract<MatchServerMessage, { type: 'sync-player' }>['player'];
...
function isSamePlayerSyncState(previous: SyncPlayerState | null, next: SyncPlayerState) {
  if (!previous) return false;
  return previous.alive === next.alive
    && previous.boardDanger === next.boardDanger
    && previous.pendingGarbage === next.pendingGarbage
    && previous.badgePoints === next.badgePoints
    && previous.badges === next.badges
    && previous.targetMode === next.targetMode
    && previous.marginBonus === next.marginBonus
    && areStringArraysEqual(previous.targetIds, next.targetIds);
}
...
const IncomingGarbageRail = memo(function IncomingGarbageRail({
  packets,
  alivePlayers,
  className,
  cellSize = 18,
        ...
        return (
          ...
          >
            {Array.from({ length: packet.lines }, (_, index) => (
              <div
                key={`${packet.id}-${index}`}
                className={`${styles.playerCell} ${styles.minoCellFilled} ${styles.garbageBlock} ${!packet.active ? styles.garbageBlockQueued : ''} ${packet.active ? styles.garbageBlockActive : ''} ${toneClass} ${packet.active && packet.progress >= 1 ? styles.garbageBlockFlashing : ''}`}
                style={{
      ...
      <div className={styles.microBoard}>
        {visibleBoard.flatMap((row, y) =>
          row.map((cell, x) => (
            <div
              key={`${bot.id}-${y}-${x}`}
              className={`${styles.playerCell} ${cell ? styles.minoCellFilled : ''} ${styles.microCell}`}
...
const PreviewShape = memo(function PreviewShape({
  type,
  cellSize = 10,
  className,
}: {
      ...
      {display.flatMap((row, rowIndex) =>
        row.map((cell, cellIndex) => (
          <div
            ...
            className={`${styles.previewCell} ${cell ? `${styles.playerCell} ${styles.minoCellFilled}` : ''}`}
            style={{
      ...
      {displayBoard.flatMap((row, y) =>
        row.map((cell, x) => {
          ...
          return (
            <div
              key={`${y}-${x}`}
              className={`${styles.playerCell} ${pieceType ? `${styles.minoCellFilled} ${styles.playerCellFilled}` : ''} ${isGhost ? styles.playerCellGhost : ''}`}
              style={style}
            />
          );
        }),
...
export default function Tetris99GameProper() {
  ...
  const lastPlayerSyncStateRef = useRef<SyncPlayerState | null>(null);
  ...
  function pushPlayerStateToServer() {
    ...
    const nextPlayerState: SyncPlayerState = {
    ...
    if (isSamePlayerSyncState(lastPlayerSyncStateRef.current, nextPlayerState)) return;
    lastPlayerSyncStateRef.current = nextPlayerState;
  ...
  function resetGame() {
    ...
    lastPlayerSyncStateRef.current = null;
